...
function dispatchNotifyEvent(inst, eventName, value, path) {
  ...
  wrapIfNeeded(/** @type {!HTMLElement} */(inst)).dispatchEvent(new CustomEvent(eventName, { detail }));
}
...
function setupCompoundStorage(node, binding) {
  if (binding.isCompound) {
    ...
    if (binding.literal && binding.kind == 'property') {
      ...
      // `innerHTML`.
      if (target === 'className') {
        // This needs to *always* wrap since an unpatched node may need
        // its className scoped.
        node = wrap(node);
      }
  ...
  class PropertyEffects extends propertyEffectsBase {
    ...
    _removeBoundDom(dom) {
      ...
      for (let i=0; i<nodes.length; i++) {
        let node = nodes[i];
        wrapIfNeeded(wrapIfNeeded(node).parentNode).removeChild(node);
      }
    }

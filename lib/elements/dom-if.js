...
import { wrap, wrapLocal } from '../utils/wrap.js';
...
class DomIfBase extends PolymerElement {
  ...
  disconnectedCallback() {
    ...
    const parent = wrapLocal(this).parentNode;
    if (!parent || (parent.nodeType == Node.DOCUMENT_FRAGMENT_NODE &&
        !wrapLocal(parent).host)) {
   ...
   */
  __ensureInstance() {
    let parentNode = wrapLocal(this).parentNode;
    if (!this.__hasInstance()) {
      ...
      if (children && children.length) {
        ...
        let lastChild = wrapLocal(this).previousSibling;
        if (lastChild !== children[children.length-1]) {
          for (let i=0, n; (i<children.length) && (n=children[i]); i++) {
            wrapLocal(parentNode).insertBefore(n, this);
          }
        }
      }
    }
...
class DomIfFast extends DomIfBase {
  ...
  __createAndInsertInstance(parentNode) {
    ...
    wrapLocal(parentNode).insertBefore(this.__instance, this);
  }
...
class DomIfLegacy extends DomIfBase {
  ...
  __createAndInsertInstance(parentNode) {
    ...
    wrapLocal(parentNode).insertBefore(this.__instance.root, this);
  }
  ...
  __teardownInstance() {
    if (this.__instance) {
      ...
      if (c$ && c$.length) {
        ...
        let parent = wrapLocal(c$[0]).parentNode;

...
/**
 * Setting to enable `customElements.polyfillLazyDefine` when the Polymer function
 * is used to define elements. When in use, the `Polymer` function will not
 * return the defined element class.
 */
export let lazyDefine =
  window.Polymer && window.Polymer.lazyDefine || false;

/**
 * Sets `lazyDefine` globally for all elements to enable
 * `customElements.polyfillLazyDefine` when the `Polymer` function is used
 * to define elements.
 *
 * @param {boolean} useLazyDefine enable or disable legacy optimizations
 * includes and url rewriting
 * @return {void}
 */

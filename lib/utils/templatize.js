...
import { wrap, wrapIfNeeded } from './wrap.js';
    ...
    if (Boolean(hide) != Boolean(n.__hideTemplateChildren__)) {
      ...
      // remove and replace slot
        ...
        if (hide) {
          ...
          wrapIfNeeded(wrapIfNeeded(n).parentNode).replaceChild(n.__polymerReplaced__, n);
        } else {
          const replace = n.__polymerReplaced__;
          if (replace) {
            wrapIfNeeded(wrapIfNeeded(replace).parentNode).replaceChild(n, replace);
          }
        }
      }
      // hide and show nodes
...
export function modelForElement(template, node) {
  ...
  while (node) {
    ...
    if ((model = node.__dataHost ? node : node.__templatizeInstance)) {
      ...
      node = wrapIfNeeded(node).parentNode;
    }
  }
  return null;
}
